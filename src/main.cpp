...
#if defined(NDEBUG)
# error "Dipiu cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x68338e9efb371f2da61b3fb69e6b375a363dc3f83cc4c109c5862bbda1ea9866");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dipiu: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dipiu Signed Message:\n";
...
{
    // Dipiu: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if(nHeight < 1000000000)  
    {
        nSubsidy = 1 * COIN;
    }
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 5500000000); // Clonecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Dipiu: 5 minutes
static const int64 nTargetSpacing = 1 * 60; // Dipiu: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dipiu: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dipiu: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x68338e9efb371f2da61b3fb69e6b375a363dc3f83cc4c109c5862bbda1ea9866");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/May/2018 Kevin Oh : Dipiu Coin developers";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1527139220;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3566054;
        ...
        assert(block.hashMerkleRoot == uint256("0xa2eef1de05aa2a3fa6b03a91ce8669938b3f1bee1f09477ba8190eb52d61e405"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xdb }; // Dipiu: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DipiuMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DipiuMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DipiuMiner(CWallet *pwallet)
{
    printf("DipiuMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dipiu-miner");
    ...
    try { loop {
        ...
        printf("Running DipiuMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DipiuMiner, pwallet));
}
